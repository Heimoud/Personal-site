// ===== WOMA PÊCHE – Script (menu, year, smooth scroll, i18n, compact switcher) =====
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Set HTML lang/dir and RTL helper class
  ...
  function applyTranslations(lang) {
    ...
    $('[data-i18n]').forEach(el => {
      ...
      if (typeof val === 'string') {
        el.innerHTML = val; // allow <strong> etc.
      }
    });
  }

  // ---- Compact option labels on phones (auto) ----
  ...
  function setSelectLabels() {
    ...
    const map = isPhone ? SHORT : FULL;
    const current = langSelect.value;
    Array.from(langSelect.options).forEach(opt => {
      if (map[opt.value]) opt.text = map[opt.value];
    });
    langSelect.value = current; // keep current selection (iOS safe)
  }
  ...
  function initLang() {
    ...
    if (langSelect) {
      ...
      langSelect.addEventListener('change', () => {
        ...
        setSelectLabels(); // keep compact/full labels in sync after change
      });
    }
    setDirByLang(saved);
  ...
  window.addEventListener("resize", setSelectLabels);
  window.addEventListener("orientationchange", setSelectLabels);
